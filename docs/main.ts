...
const globalRefs: Array<{ source_name: string; url: string }> = [];
// source_name → 1-based global reference numbers for that source
const sourceGlobalNums = new Map<string, number[]>();
...
async function init() {
  ...
  if (initialTab === 'dictionary' || initialTab === 'corpus' || initialTab === 'references') switchTab(initialTab);
}
...
function switchTab(name: 'dictionary' | 'corpus' | 'references') {
...
function buildSentenceEl(sentence: CorpusSentence): HTMLDivElement {
  ...
  if (sentence.source) {
    ...
    if (nums?.length) {
      ...
      nums.forEach((n, i) => {
        ...
        src.appendChild(a);
        if (i < nums.length - 1) src.appendChild(document.createTextNode(' '));
      });
    }
    div.appendChild(src);
  }
  ...
  copyScript.addEventListener('click', () => {
    const text = sentence.tokens.map(tok =>
      'punctuation' in tok ? tok.punctuation :
...
function computeLinzklaPct(tokens: import('./types.js').Token[], overrides?: string[]): string {
  let cjk = 0, hira = 0;
